...
Press `q` to quit, `Tab` to move focus, `f` to toggle follow mode for the
selected source, and `?` to open the help screen. Use the level selector to
filter the selected source.
...
- Sources are edited in the TOML file rather than inside the app.
